...
const generateToken = async function (req, userData) {

    let userAgent = useragent.parse(req.headers['user-agent']);
    // console.log(userAgent);
    try {

        let cipher = CryptoJS.AES.encrypt('' + userData.id, config.SECRET_KEY); //Encrypt user ID
        cipher = cipher.toString();

        const token = jwt.sign({
            user: cipher,
            sessionId: userData.newSessionId,
